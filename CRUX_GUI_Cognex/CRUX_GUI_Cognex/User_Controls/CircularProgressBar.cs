...
namespace CRUX_GUI_Cognex.User_Controls
{
    ...
    public class CPB : Control, ITimerManager
    {
        ...
        #region Progress shape is Only Infinity
        [Description("Action Delay"), Category("Behavior")]
        ...
        public void TimerStart()
        {
            while (true)
            {
                if (TokenSource.Token.IsCancellationRequested == true)
                    break;
                if (_ProgressShape.Infinity == this.ProgressShapeVal/* || _ProgressShape.Round == this.ProgressShape*/)
                {
                    //Increment(1);         
                    Thread.Sleep(_Delay);
                    //if (Value >= 100)
                ...
                else
                {
                    if (_ProgressAction)
                    {
                        ...
                        else
                        {
                            Bar1 += 3;
                            Bar2 += 3;
                            Bar1 = Bar1 > 255 ? 255 : Bar1;
                            Bar2 = Bar2 > 255 ? 255 : Bar2;
                            BarColor1 = Color.FromArgb(Bar1, BarColor1.R, BarColor1.G, BarColor1.B);
                            BarColor2 = Color.FromArgb(Bar2, BarColor2.R, BarColor2.G, BarColor2.B);
                        }
                    }

...
// 	}
	...
	SetupComm(m_hComm, dwbaud, dwbaud);
...
BOOL CCommThread::OpenPort(char *pPortName, int nBaud, WORD wPortID,BYTE byParity,void (*callback)(int nPort))
{
	...
	DWORD	dwbaud;
	char cpPortname[20];
	//(wPortID>4) sprintf_s(pPortName,sizeof(pPortName),"\\\\.\\COM%d",wPortID);
	sprintf_s(cpPortname, sizeof(cpPortname), "\\\\.\\COM%d", wPortID);
...
// 	}
	...
	if(!(m_osWrite.hEvent = CreateEvent(NULL,TRUE,FALSE,NULL)))
		return FALSE;
	strcpy_s(m_pPortName, cpPortname);
	m_hComm = CreateFile(m_pPortName,GENERIC_READ | GENERIC_WRITE,0,NULL,
		OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL|FILE_FLAG_OVERLAPPED,NULL);
	if(m_hComm==(HANDLE)-1) return FALSE;
	SetCommMask(m_hComm,EV_RXCHAR);
	SetupComm(m_hComm, nBaud, nBaud);
	PurgeComm(m_hComm,PURGE_TXABORT|PURGE_TXCLEAR|PURGE_RXABORT|PURGE_RXCLEAR);
	/*timeouts.ReadIntervalTimeout = 0xFFFFFFFF;
	timeouts.ReadTotalTimeoutConstant = 0;
	timeouts.ReadTotalTimeoutMultiplier = 0;
	...
	dcb.BaudRate = dwbaud;
	dcb.ByteSize = 8;
	dcb.Parity = NOPARITY;  //0:none 1: odd 2: even 3: mark 4: space

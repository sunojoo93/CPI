...
BOOL CVSLightTaskDlg::OnInitDialog()
{
	...
	// 버전 표기
	CString title;
	title.Format(_T("TrivisionLight Light %s"), _Version);
	SetWindowText(title);
...
void CVSLightTaskDlg::OnBnClickedButtonTest()
{
	...
	stTemp.stLight[1].nChCnt = 8;
	for (int i = 0; i < stTemp.stLight[0].nChCnt; i++)
		stTemp.stLight[0].nLightVal[i] = 20;
	for (int i = 0; i < stTemp.stLight[1].nChCnt; i++)
		stTemp.stLight[1].nLightVal[i] = 20;
...
void CVSLightTaskDlg::OnBnClickedButtonMultiOn()
{
	...
	STRU_LIGHT_INFO stTemp;
	stTemp.nControllerCount = 1;
	stTemp.stLight[0].nChCnt = 4;
	stTemp.stLight[1].nChCnt = 4;
	for (int i = 0; i < stTemp.stLight[0].nChCnt; i++)
		stTemp.stLight[0].nLightVal[i] = 200;
	for (int i = 0; i < stTemp.stLight[1].nChCnt; i++)
		stTemp.stLight[1].nLightVal[i] = 200;
	theApp.m_pLight->MultiTurnOn(stTemp);
}
...
void CVSLightTaskDlg::OnBnClickedButtonMultiOff()
{
	...
	stTemp.stLight[1].nChCnt = 4;
	for (int i = 0; i < stTemp.stLight[0].nChCnt; i++)
		stTemp.stLight[0].nLightVal[i] = 0;
	for (int i = 0; i < stTemp.stLight[1].nChCnt; i++)
		stTemp.stLight[1].nLightVal[i] = 0;
	theApp.m_pLight->MultiTurnOn(stTemp);
}

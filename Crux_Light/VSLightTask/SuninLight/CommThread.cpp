...
BOOL CCommThread::OpenPort(char *pPortName, int nBaud, WORD wPortID,BYTE byParity,HWND hCommWnd)
{
	...
	if(wPortID>4) sprintf_s(pPortName,10,"\\\\.\\COM%d",wPortID);
	...
	m_hComm = CreateFile(CString(m_pPortName),GENERIC_READ | GENERIC_WRITE,0,NULL,
...
BOOL CCommThread::OpenPort(char *pPortName, int nBaud, WORD wPortID,BYTE byParity,void (*callback)(int nPort,char *pBuff))
{
	COMMTIMEOUTS timeouts;
	DCB		dcb;
	DWORD	dwThreadID;
...
DWORD ThreadWatchComm(CCommThread* pComm)
{
	...
	if(!bOk)
	{
		MessageBox(NULL,_T("Error while creating ThreadWatchComm"),NULL,MB_OK);
		return FALSE;
	}
		...
		if((dwEvent & EV_RXCHAR) == EV_RXCHAR)
		{
			do
			{
				...
				if(BUFF_SIZE - rxByte > (int)dwRead)
				{
					...
					MessageBox(NULL,_T("m_QueueRead FULL!"),NULL,MB_OK);
			...
			if (size)
			{
				...
				{
					pComm->m_QueueRead.GetByte(&pBuff[nCmdRxCnt]);
					nCmdRxCnt++;
					if(pBuff[nCmdRxCnt-1] == 0x0D)	// 캐리지리턴
					{
						if (pComm->m_CallBackFuncPort)
						{
							pComm->m_CallBackFuncPort(pComm->m_wPortID, (char*) pBuff);
							nCmdRxCnt = 0;
						}
					}
					else if (pBuff[nCmdRxCnt-1] == 0x0A)	// 뉴라인
					{
						nCmdRxCnt = 0;
					}
					else if (nCmdRxCnt > 1000)
					{
						nCmdRxCnt = 0;
					}
				}
			...
			//if(pComm->m_QueueRead.GetSize() && pComm->m_CallBackFuncPort) pComm->m_CallBackFuncPort(pComm->m_wPortID, (char*) buff);

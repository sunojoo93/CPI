...
BOOL CVSLightTaskDlg::OnInitDialog()
{
	...
	// 버전 표기
	CString title;
	title.Format(_T("Sunin Light %s"), _Version);
	SetWindowText(title);
...
void CVSLightTaskDlg::OnBnClickedButtonTest()
{
	...
	for(int i = 0 ; i < 17 ; i++)
		stTemp.stLight->nLightVal[i] = i;

	stTemp.stLight[1].nLightVal[0] = 18;
	stTemp.stLight[1].nLightVal[1] = 19;
	stTemp.stLight[1].nLightVal[2] = 20;

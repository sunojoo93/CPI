...
LLightController::LLightController()
{ 
	m_CallBack	= NULL;
	m_hWnd		= NULL;
	m_pCommLLight= new CRs232CommSTD;
	memset(&m_stLightStatus,NULL,sizeof(m_stLightStatus));
}

LLightController::~LLightController()
{
	if(m_pCommLLight) delete ((CRs232CommSTD *)m_pCommLLight);
	m_pCommLLight= NULL;
	m_CallBack	= NULL;
	m_hWnd		= NULL;
}

void LLightController::SetOnOff(LIGHT_LLight_STATUS stLightInfo)
{
	bool bChanged = false;
	CString strOnOff = _T("");
	...
	if (bChanged)
	{
		char pBuff[PROTOCOL_LLight_LENGTH];
		sprintf(pBuff,"setonex %d %s%c", GetCtrID(), strOnOff, '\r');
		((CRs232CommSTD *)m_pCommLLight)->WriteData((unsigned char *)pBuff,strlen(pBuff));
		Sleep(PROTOCOL_LLight_WAIT);
	}
}

void LLightController::GetOnOffAll()
{
	char pBuff[PROTOCOL_LLight_LENGTH];
...
void LLightController::GetError()
{
	char pBuff[PROTOCOL_LLight_LENGTH];
...
void LLightController::SetBrightness(LIGHT_LLight_STATUS stLightInfo)
{
	bool bChanged = false;
	...
	{
		char pBuff[PROTOCOL_LLight_LENGTH];
		strcpy_s(pBuff, PROTOCOL_LLight_LENGTH, CW2A(strBrightness));
...
void LLightController::GetBrightnessAll()
{
	char pBuff[PROTOCOL_LLight_LENGTH];
...
void LLightController::GetCurrentAll()
{
	char pBuff[PROTOCOL_LLight_LENGTH];
...
void LLightController::GetTemperatureAll()
{
	char pBuff[PROTOCOL_LLight_LENGTH];
...
int LLightController::Create(int nPort, int nBaudRate)
{
	int ret;
	char buff[10];
	sprintf(buff,"COM%d",nPort);
	
	ret = ((CRs232CommSTD *)m_pCommLLight)->Create(buff,nBaudRate,nPort,false,CallBackFunc);
	return ret;
}

int LLightController::Create(int nPort,int nBaudRate,void (*callback)(int nPort,char *pBuff))
{
	int ret;
	char buff[10];
	sprintf(buff,"COM%d",nPort);
...
int LLightController::Create(int nPort,int nBaudRate,HWND hWnd)
{
	int ret;
	char buff[10];
	sprintf(buff,"COM%d",nPort);

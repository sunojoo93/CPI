#include "stdafx.h"
#include "LLightLightControl.h"
#include "LLightLight/Library/ProtocolLibRegLLight.h"
...
{
	((CLLightLight*)theApp.m_pLight)->CallBackFunc_FromUnit(nPort, pBuff);
}


CLLightLight::CLLightLight()
	//: CDialog(CLLightLight::IDD)
...
// Turn On
void CLLightLight::MultiTurnOn(STRU_LIGHT_INFO stLightInfo)
{
	if (!m_bIsPortOpen)
	{
	...
	LLightDll::SetBrightness(m_nPort, 0, LightVal, m_nChannelTotalCnt);
...
// 사용 안함
void CLLightLight::TurnOn(UINT nController, UINT nChannel, UINT nIntensity)
{
	if (!m_bIsPortOpen)
	{
		theApp.WriteLog(_T("MultiTurnOn Fail. Port is not open."));
	...
	//LLightDll::SetBrightness(m_nPort, nChannel, nIntensity);	
	
}

void CLLightLight::GetC(UINT nChannel)
{
// 	if (!m_bIsPortOpen)
// 	{
...
// Turn Off
void CLLightLight::MultiTurnOff()
{
	if (!m_bIsPortOpen)
	{
		theApp.WriteLog(_T("MultiTurnOn Fail. Port is not open."));
	...
	{
		LLightDll::SetBrightness(m_nPort, n+1, LightVal, m_nChannelTotalCnt);
	}*/

	// LLight는 ctrl no 안씀, 485 통신에서나 쓸듯한데 485를 안씀.
	LLightDll::SetBrightness(m_nPort, 0, LightVal, m_nChannelTotalCnt);
...
// 사용 안함
void CLLightLight::TurnOff(UINT nController, UINT nChannel)
{
	if (!m_bIsPortOpen)
	{
		theApp.WriteLog(_T("MultiTurnOn Fail. Port is not open."));
	...
	// nController 는 사용 않함.. 확인 필요. Todo..
	//LLightDll::SetBrightness(m_nPort, nChannel, 0);	
}

void CLLightLight::CheckLight(int nControllerCount)
{
	if (!m_bIsPortOpen)
	{
		theApp.WriteLog(_T("MultiTurnOn Fail. Port is not open."));
	...
	LLightDll::GetError(m_nPort);
}

void CLLightLight::GetBrightness()
{
	if (!m_bIsPortOpen)
	{
		theApp.WriteLog(_T("MultiTurnOn Fail. Port is not open."));
	...
	// nController 는 사용 않함.. 확인 필요. Todo..
	LLightDll::GetBrightness(0);	
}



BOOL CLLightLight::InitializeLight(CString strInitFilePath)
{
	int nRet = APP_OK;
	...
	*/
	...
	// 현재 LLight 조명 한가지 타입 사용(MODULE_16CH)
	// 'MODULE_16CH' : 컨트롤러당 최대 채널 개수 17개(모듈 설비 총 1개 컨트롤러 17채널 사용. 17(CtrlID_1))
	if(nMaxChCnt == MODULE_16CH)			m_nChannelType = MODULE_16CH;

	if (m_bIsPortOpen == TRUE)
	{
		nRet = LLightDll::ClosePort(m_nPort);
		if (nRet != APP_OK)
		{
	...
	switch(LLightDll::OpenPort(m_nPort, m_nControllerCnt, m_nBaudRate, m_nCtrlChannelCnt, CallBackFunc))
	{
	case SUCCESS:
...
UINT CLLightLight::ThreadCheckLight(LPVOID lParam)
{ 
	//theApp.m_pInternalServer->m_fnCmdEditSend()
	CLLightLight* pThis = (CLLightLight*)lParam;
...
void CLLightLight::CallBackFunc_FromUnit(int nPort,char *pBuff)
{
	CString strTemp;
	CString strOrg=_T("");
	char* token = NULL;

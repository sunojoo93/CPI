...
namespace Snappy.Sharp
{
    public static class Snappy
    {
        ...
        public static byte[] Uncompress(byte[] compressed, int offset = 0)
        {
            var decompressor = new SnappyDecompressor();
            int[] nComLen =  decompressor.ReadUncompressedLength(compressed, offset);
            return decompressor.Decompress(compressed, offset, compressed.Count());
            throw new NotImplementedException();
        }

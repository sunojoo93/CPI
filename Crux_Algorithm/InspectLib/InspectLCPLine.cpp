...
long CInspectLCPLine::DoFindLCPLineDefect(cv::Mat matSrcBuffer, cv::Mat& matBKBuffer, cv::Mat& matDarkBuffer, cv::Mat& matBrightBuffer, cv::Mat matMask_Active,
	cv::Point* ptCorner, cv::Rect rcAlignCellROI, double* dAlignPara, cv::Rect* rcCHoleROI, double* dPara, int* nCommonPara, CString strAlgPath, CString strBubblePath, CString strBubblePath_Menual, ENUM_INSPECT_MODE eInspMode, STRU_DEFECT_ITEM* EngineerDefectJudgment, CDefectCCD *cCCD, cv::Mat* matCholeBuffer)
{
	...
	case E_IMAGE_CLASSIFY_AVI_SIDE:
	{
		...
		nErrorCode = LCP_Line(matSrcBuffer, matDstImage, matMask_Active, rectROI, rcAlignCellROI, dPara, nCommonPara, strAlgPath, strBubblePath, strBubblePath_Menual, eInspMode, EngineerDefectJudgment);
	...
	writeInspectLog(E_ALG_TYPE_AVI_LINE, __FUNCTION__, _T("Result Copy."));

	// 할당 해제
	matDstImage[E_DEFECT_COLOR_DARK].release();
	matDstImage[E_DEFECT_COLOR_BRIGHT].release();

	writeInspectLog(E_ALG_TYPE_AVI_LINE, __FUNCTION__, _T("End.Dispersion"));

	return nErrorCode;
}

long CInspectLCPLine::LCP_Line(cv::Mat& matSrcImage, cv::Mat* matDstImage, cv::Mat matMask_Active, CRect rectROI, cv::Rect rcAlignCellROI, double* dPara, int* nCommonPara, CString strAlgPath, CString strBubblePath, CString strBubblePath_Menual, ENUM_INSPECT_MODE eInspMode, STRU_DEFECT_ITEM* EngineerDefectJudgment)
{
	long	nErrorCode = E_ERROR_CODE_TRUE;
	// 공통 파라미터
	...
	//////////////////////////////////////////////////////////////////////////
	int nMask_Down_Sz = (int)dPara[E_PARA_LCP_LINE_AREA_MASK_SIZE_DOWN];
	//////////////////////////////////////////////////////////////////////////
	...
	cv::Mat matDstTemp_Dark[2];

	matDstTemp_Dark[E_DEFECT_COLOR_DARK] = cMatBufTemp.GetMat(matDstImage[E_DEFECT_COLOR_DARK].size(), matDstImage[E_DEFECT_COLOR_DARK].type(), false);
	matDstTemp_Dark[E_DEFECT_COLOR_BRIGHT] = cMatBufTemp.GetMat(matDstImage[E_DEFECT_COLOR_BRIGHT].size(), matDstImage[E_DEFECT_COLOR_BRIGHT].type(), false);

	cv::Mat matDstTemp_Bright[2];

	matDstTemp_Bright[E_DEFECT_COLOR_DARK] = cMatBufTemp.GetMat(matDstImage[E_DEFECT_COLOR_DARK].size(), matDstImage[E_DEFECT_COLOR_DARK].type(), false);
	matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT] = cMatBufTemp.GetMat(matDstImage[E_DEFECT_COLOR_BRIGHT].size(), matDstImage[E_DEFECT_COLOR_BRIGHT].type(), false);


	cv::Mat matSrcTemp_Bright;

	cv::Mat matActive;
	matMask_Active.copyTo(matActive);
	Mask_SizeDown(matActive, nMask_Down_Sz);

	MaximumFilter(matSrcImage, matSrcTemp_Bright, 5);

	RangeAvgThreshold(matSrcImage, matDstTemp_Dark, rectTemp, dPara, cMem);
	RangeAvgThreshold(matSrcTemp_Bright, matDstTemp_Bright, rectTemp, dPara, cMem);

	if (bImageSave)
	{
		CString strTemp;
		strTemp.Format(_T("%s\\%02d_%02d_%02d_LINE_%02d_LCPLine_RangeThreshold_Dark.jpg"), strAlgPath, nImageNum, nCamNum, nROINumber, nSaveImageCount++);
		ImageSave(strTemp, matDstTemp_Dark[E_DEFECT_COLOR_DARK]);
	}

	
	cv::bitwise_and(matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT], matActive, matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT]);
	Mask_SizeUp(matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT], 20);

	if (bImageSave)
	{
		CString strTemp;
		strTemp.Format(_T("%s\\%02d_%02d_%02d_LINE_%02d_LCPLine_RangeThreshold_Bright.jpg"), strAlgPath, nImageNum, nCamNum, nROINumber, nSaveImageCount++);
		ImageSave(strTemp, matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT]);

	}

	cv::subtract(matDstTemp_Dark[E_DEFECT_COLOR_DARK], matDstTemp_Bright[E_DEFECT_COLOR_BRIGHT], matDstTemp_Dark[E_DEFECT_COLOR_DARK]);

	matDstTemp_Dark[E_DEFECT_COLOR_DARK].copyTo(matDstImage[E_DEFECT_COLOR_DARK]);

	if (bImageSave)
	{
		CString strTemp;
		strTemp.Format(_T("%s\\%02d_%02d_%02d_LINE_%02d_LCPLine_RangeThreshold_Subtract.jpg"), strAlgPath, nImageNum, nCamNum, nROINumber, nSaveImageCount++);
	...
	{
		CString strTemp;
		strTemp.Format(_T("%s\\%02d_%02d_%02d_LINE_%02d_LCPLine_Active_Mask.jpg"), strAlgPath, nImageNum, nCamNum, nROINumber, nSaveImageCount++);
		ImageSave(strTemp, matDstImage[E_DEFECT_COLOR_DARK]);
	}
...
// 	int labelCount = 0;
// 
// 	labelCount = First_blob_Line(matSrcImage, matDstImage[E_DEFECT_COLOR_DARK], matLabels, stats_1, centroids, rectTemp_Dust, dPara);
// 
// 	if (bImageSave)
// 	{
// 		CString strTemp;
// 		strTemp.Format(_T("%s\\%02d_%02d_%02d_Line_%02d_Dark_First_blob.bmp"), strAlgPath, nImageNum, nCamNum, nROINumber, nSaveImageCount++);
// 		ImageSave(strTemp, matDstImage[E_DEFECT_COLOR_DARK]);
// 	}
...
long CInspectLCPLine::RangeAvgThreshold(cv::Mat& matSrcImage, cv::Mat* matDstImage, CRect rectROI, double* dPara, CMatBuf* cMemSub)
{
	// 에러 코드
	long	nErrorCode = E_ERROR_CODE_TRUE;
...
// 분할 영역 이진화
long CInspectLCPLine::RangeAvgThreshold_8bit(cv::Mat& matSrcImage, cv::Mat* matDstImage, CRect rectROI, double* dPara, CMatBuf* cMemSub)
{
	long	nBlurLoop_5x5 = (long)dPara[E_PARA_LCP_LINE_COMMON_BLUR_LOOP];
	long	nBlur_Size = (long)dPara[E_PARA_LCP_LINE_COMMON_BLUR_LOOP_SIZE];

	long	nSegX = (long)dPara[E_PARA_LCP_LINE_COMMON_SEG_X];
	long	nSegY = (long)dPara[E_PARA_LCP_LINE_COMMON_SEG_Y];
	long	nEdgeArea = (long)dPara[E_PARA_LCP_LINE_COMMON_EDGE_AREA];

	float	fActiveDarkRatio = (float)dPara[E_PARA_LCP_LINE_COMMON_ACTIVE_DARK_RATIO];
	float	fActiveBrightRatio = (float)dPara[E_PARA_LCP_LINE_COMMON_ACTIVE_BRIGHT_RATIO];
	float	fEdgeDarkRatio = (float)dPara[E_PARA_LCP_LINE_COMMON_EDGE_DARK_RATIO];
	float	fEdgeBrightRatio = (float)dPara[E_PARA_LCP_LINE_COMMON_EDGE_BRIGHT_RATIO];

	int		nMinThGV = (long)dPara[E_PARA_LCP_LINE_COMMON_MIN_THRESHOLD];
	int		nMedian = (long)dPara[E_PARA_LCP_LINE_COMMON_MEDIAN];
...
long CInspectLCPLine::RangeAvgThreshold_16bit(cv::Mat& matSrcImage, cv::Mat* matDstImage, CRect rectROI, double* dPara, CMatBuf* cMemSub)
{
	long	nBlurLoop_5x5 = (long)dPara[E_PARA_LCP_LINE_COMMON_BLUR_LOOP];
	long	nBlur_Size = (long)dPara[E_PARA_LCP_LINE_COMMON_BLUR_LOOP_SIZE];
...
long CInspectLCPLine::GetHistogram(cv::Mat& matSrcImage, cv::Mat& matHisto)
{
	// 버퍼가 없는 경우.
...
long CInspectLCPLine::GetAverage_From_Histo(cv::Mat matHisto, int nMin, int nMax, double &dblAverage)
{
	double dblValSum = 0;
...
long CInspectLCPLine::GetMeanStdDev_From_Histo(cv::Mat matHisto, int nMin, int nMax, double &dblAverage, double &dblStdev)
{
...
long CInspectLCPLine::MedianFilter(cv::Mat& matSrcBuf, cv::Mat& matDstBuf, int nKSize, CMatBuf* cMemSub)
{
...
void CInspectLCPLine::Insp_RectSet(cv::Rect& rectInspROI, CRect& rectROI, int nWidth, int nHeight, int nOffset)
{
...
long CInspectLCPLine::ImageSave(CString strPath, cv::Mat matSrcBuf)
{
	if (matSrcBuf.type() == CV_8U)
	{
...
//Blob
long CInspectLCPLine::GetDefectList(cv::Mat matSrcBuffer, cv::Mat MatColorBuffer, cv::Mat matDstBuffer[2], cv::Mat& matDrawBuffer,
	cv::Point* ptCorner, double* dPara, int* nCommonPara, CString strAlgPath, STRU_DEFECT_ITEM* EngineerDefectJudgment, stDefectInfo* pResultBlob)
{
	...
	// 불량 판정 & 결과 보내기
	{
		...
		writeInspectLog(E_ALG_TYPE_AVI_LINE, __FUNCTION__, _T("BlobJudgment (DARK)."));
		...
		writeInspectLog(E_ALG_TYPE_AVI_LINE, __FUNCTION__, _T("BlobJudgment (BRIGHT)."));
	}
...
long CInspectLCPLine::Image_Pow(int ImgType, double dpow, Mat& InPutImage, Mat& OutPutImage)
{
...
long CInspectLCPLine::Binary_16bit(cv::Mat& matSrcBuf, cv::Mat& matDstBuf, double thresh, bool bInv, CMatBuf* cMemSub)
{
	// 버퍼가 없는 경우
	if (matSrcBuf.empty())		return E_ERROR_CODE_EMPTY_BUFFER;
...
long  CInspectLCPLine::C_Mexican_filter(cv::Mat& ip, int sz, int nMx_blur_sz, int nMx_blur_sigma) {
	int r = (sz - 1) / 2;
...
float*  CInspectLCPLine::computeKernel2D(int r) {
	int sz = 2 * r + 1;
...
boolean  CInspectLCPLine::convolveFloat(cv::Mat& ip, float* kernel, int kw, int kh, int sz) {
...
double CInspectLCPLine::getScale(float* kernel, int sz) {
	double scale = 1.0;
	double sum = 0.0;
	for (int i = 0; i < sz*sz; i++)
		sum += kernel[i];
...
void CInspectLCPLine::Mask_SizeDown(cv::Mat matSrc, int nUpSize)
{
	cv::Mat Temp16;
...
void CInspectLCPLine::Mask_SizeUp(cv::Mat matSrc, int nDownSize)
{
	cv::Mat matTemp;
	cv::Mat Temp16;
...
int CInspectLCPLine::First_blob_Line(cv::Mat Srcimg, cv::Mat DstImg, cv::Mat matLabels, cv::Mat stats, cv::Mat centroids, cv::Rect rectTemp_Dust, double* dPara)
{
	float	fAxis_Ratio = (float)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_AXIO_RATIO];
	float	fArea_Ratio = (float)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_AREA_RATIO];
	long	nArea_Min = (long)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_AREA_MIN];
	long	nArea_Max = (long)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_AREA_MAX];
	long	nJudge1_Flag = (long)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_SPEC1_USE];
	long	nJudge1_Area_Min = (long)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_SPEC1_AREA_MIN];
	float	fJudge1_Axis_Ratio = (float)dPara[E_PARA_LCP_LINE_AREA_FIRST_JUDGE_SPEC1_AXIO_RATIO];
...
void CInspectLCPLine::MinimumFilter(cv::Mat src, cv::Mat& dst, int nMaskSize)
{
	Mat prc;
	src.copyTo(prc);
...
void CInspectLCPLine::MaximumFilter(cv::Mat src, cv::Mat& dst, int nMaskSize)
{
	Mat prc;
	src.copyTo(prc);

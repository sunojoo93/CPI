...
// 데이터 받아오기
int gdsToText(const std::string &fileName, vector<stGDSData>& vecStGDSData)
{
	ifstream gdsFile;
	...
	do {
		...
		if (gdsRecordToText(readBlk, vecStGDSData)) {
			cout << "GDS read error" << endl;
			break;
		}
	} while (hexKey != GDS_ENDLIB);
...
int gdsRecordToText(char *recIn, vector<stGDSData>& vecStGDSData)
{
	uint32_t GDSKey;
	bitset<16> bitarr;
	vector<int> integer;
	...
	/*	cout << ":{"
		...
		{
			stGDSData GDSData;
			GDSData.nLayer = integer[0];
			vecStGDSData.push_back(GDSData);
		}
		else if (keyName == "XY")
		{
			int nSize = vecStGDSData.size();
			int nDataSize = integer.size();
			vecStGDSData[nSize - 1].vecPointdXY.resize(nDataSize / 2);
			for (unsigned int j = 0; j < nDataSize; j++) {
				if (nSize != 0) {
					if ((j % 2 == 0 || j == 0) && j != 1)
						vecStGDSData[nSize - 1].vecPointdXY[j / 2].x = integer[j];
					else if (j % 2 == 1 || j == 1)
						vecStGDSData[nSize - 1].vecPointdXY[j / 2].y = integer[j];
				}
			}

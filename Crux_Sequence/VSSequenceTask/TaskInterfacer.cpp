...
int	 TaskInterfacer::m_fnPrintLog(BOOL bSendUI, const wchar_t* pszLogData, ...)		// bSendUI : UI Log 송부 기능 사용 여부
{
	...
	if (cBuffer)
	{
		...
		if (bSendUI)
			::WorkManager::VS_Send_Log_To_UI(cBuffer, sizeof(wchar_t)*len);		// UI Log 송부 기능 추가
		//free(cBuffer);
		delete [] cBuffer;
	}

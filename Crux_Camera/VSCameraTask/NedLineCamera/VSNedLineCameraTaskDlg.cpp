...
BOOL CVSCameraTaskDlg::OnInitDialog()
{
	...
	// 버전 표기
	CString title;
	title.Format(_T("NED Line Camera %s"), _Version);
	SetWindowText(title);

	// TODO: 여기에 추가 초기화 작업을 추가합니다.
	((CNedLineCamera*)theApp.m_pCamera)->m_Camera->GetAllProperty();
	NedParam* param = &NedParam::Instance();
	SetDlgItemText(IDC_EDIT_INI_FILE_PATH, theApp.GetInitFilePath());
	SetDlgItemText(IDC_EDIT_CAM_MODEL, param->GetParamStr(_T("Type")));
	SetDlgItemText(IDC_EDIT_CAM_GRB_NO, ToString(((CNedLineCamera*)theApp.m_pCamera)->FrameGrabberNo));
	SetDlgItemText(IDC_EDIT_CAM_DIG_CH, ToString(((CNedLineCamera*)theApp.m_pCamera)->DigitizerChannel));
	SetDlgItemText(IDC_EDIT_CAM_DCF, ((CNedLineCamera*)theApp.m_pCamera)->DcfFile);
	SetDlgItemText(IDC_EDIT_CAM_PORT, ToString(((CNedLineCamera*)theApp.m_pCamera)->CameraPort));
	SetDlgItemText(IDC_EDIT_CAM_BAUD, ToString(((CNedLineCamera*)theApp.m_pCamera)->CameraBaud));
...
void CVSCameraTaskDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	switch(nIDCtl)
	{
		...
		{
			if (((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->IsConnected())
			{
				m_fnChangeBtnColor(lpDrawItemStruct, LIGHT_GREEN, DARK_GREEN);
			}
			else
	...
	if (((CNedLineCamera*)theApp.m_pCamera)->m_GrabFlag)
...
void CVSCameraTaskDlg::OnCbnSelchangeComboCam()
{
	...
	case IDC_COMBO_CAM_TRIMODE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("t", m_comboCamTriMode.GetCurSel());
		break;
	case IDC_COMBO_CAM_VDOUT:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("v", m_comboCamVdOut.GetCurSel());
		break;
	case IDC_COMBO_CAM_TAP:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("tap", m_comboCamTap.GetCurSel() + 1);
		break;
	case IDC_COMBO_CAM_TEST:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("T", m_comboCamTest.GetCurSel());
		break;
	}
}
...
void CVSCameraTaskDlg::OnBnClickedCheckCamReverse()
{
	...
	case IDC_CHECK_CAM_REVERSE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("rev", m_checkReverse.GetCheck());
		break;
	}
}
...
void CVSCameraTaskDlg::OnBnClickedButtonCam()
{
	...
	case IDC_BUTTON_CAM_LOAD:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->LoadAllProperty();
		break;
	case IDC_BUTTON_CAM_SAVE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SaveAllProperty();
		break;
	...
	case IDC_BUTTON_CAM_GRABSTART:
		((CNedLineCamera*)theApp.m_pCamera)->StartGrab(false, true);
		break;
	case IDC_BUTTON_CAM_GRABSTOP:
		((CNedLineCamera*)theApp.m_pCamera)->StopGrab();
		break;
	}
}
...
void CVSCameraTaskDlg::OnEnChangeEditCam()
{
	...
	case IDC_EDIT_CAM_GR:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("r", value);
		break;
	case IDC_EDIT_CAM_GRFO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("grfo", value);
		break;
	case IDC_EDIT_CAM_GRFE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("grfe", value);
		break;
	case IDC_EDIT_CAM_GRRO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("grro", value);
		break;
	case IDC_EDIT_CAM_GRRE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("grre", value);
		break;
	case IDC_EDIT_CAM_GG:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("g", value);
		break;
	case IDC_EDIT_CAM_GGFO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("ggfo", value);
		break;
	case IDC_EDIT_CAM_GGFE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("ggfe", value);
		break;
	case IDC_EDIT_CAM_GGRO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("ggro", value);
		break;
	case IDC_EDIT_CAM_GGRE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("ggre", value);
		break;
	case IDC_EDIT_CAM_GB:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("b", value);
		break;
	case IDC_EDIT_CAM_GBFO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("gbfo", value);
		break;
	case IDC_EDIT_CAM_GBFE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("gbfe", value);
		break;
	case IDC_EDIT_CAM_GBRO:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("gbro", value);
		break;
	case IDC_EDIT_CAM_GBRE:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("gbre", value);
		break;
	case IDC_EDIT_CAM_WBR:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("WBr", value);
		break;
	case IDC_EDIT_CAM_WBG:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("WBg", value);
		break;
	case IDC_EDIT_CAM_WBB:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("WBb", value);
		break;
	case IDC_EDIT_CAM_EXP:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("i", value);
		break;
	case IDC_EDIT_CAM_LINEDELAY:
		((CNedLineCamera*)theApp.m_pCamera)->m_Camera->SetProperty("d", value);
		break;
	}
}
...
void CVSCameraTaskDlg::OnBnClickedButtonTrigger()
{
	...
	case IDC_BUTTON_TRIGGER_INIT_F:
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->StopTriggerGen0();
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->TriggerOrigin();
		break;
	...
	case IDC_BUTTON_TRIGGER_SET_F:
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->TriggerStartPositionChannel0(
			((CNedLineCamera*)theApp.m_pCamera)->TriggerStartF, ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopF, ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodF);
		break;
	case IDC_BUTTON_TRIGGER_SET_B:
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->TriggerStartPositionChannel0(
			((CNedLineCamera*)theApp.m_pCamera)->TriggerStartB, ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopB, ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodB);
		break;
	case IDC_BUTTON_TRIGGER_SCAN_F:
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->StartForwardScan();
		break;
	case IDC_BUTTON_TRIGGER_SCAN_B:
		((CNedLineCamera*)theApp.m_pCamera)->m_Trigger->StartBackwardScan();
		break;
	case IDC_BUTTON_TRIGGER_SAVE:
		lib->SetDataToINI(_T("Trigger"), _T("StartF"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerStartF, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("StartB"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerStartB, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("StopF"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopF, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("StopB"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopB, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("PeriodF"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodF, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("PeriodB"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodB, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("CountF"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerCountF, lib->GetData(_T("inifile")));
		lib->SetDataToINI(_T("Trigger"), _T("CountB"), ((CNedLineCamera*)theApp.m_pCamera)->TriggerCountB, lib->GetData(_T("inifile")));
		break;
	}
}
...
{
	int heightPixel = ((CNedLineCamera*)theApp.m_pCamera)->m_lDigSizeY;
	if (heightPixel < 0 || heightPixel > 999999)
		heightPixel = 1000;
	((CNedLineCamera*)theApp.m_pCamera)->TriggerStartF = GetDlgItemInt(IDC_EDIT_TRIGGER_STARTPOS_F);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerStartB = GetDlgItemInt(IDC_EDIT_TRIGGER_STARTPOS_B);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodF = GetDlgItemInt(IDC_EDIT_TRIGGER_PERIOD_F);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodB = GetDlgItemInt(IDC_EDIT_TRIGGER_PERIOD_B);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerCountF = GetDlgItemInt(IDC_EDIT_TRIGGER_COUNT_F);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerCountB = GetDlgItemInt(IDC_EDIT_TRIGGER_COUNT_B);
	((CNedLineCamera*)theApp.m_pCamera)->TriggerStopF =
		((CNedLineCamera*)theApp.m_pCamera)->TriggerStartF + ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodF * ((CNedLineCamera*)theApp.m_pCamera)->TriggerCountF * heightPixel;
	((CNedLineCamera*)theApp.m_pCamera)->TriggerStopB =
		((CNedLineCamera*)theApp.m_pCamera)->TriggerStartB + ((CNedLineCamera*)theApp.m_pCamera)->TriggerPeriodB * ((CNedLineCamera*)theApp.m_pCamera)->TriggerCountB * heightPixel;
	SetDlgItemInt(IDC_EDIT_TRIGGER_STOPPOS_F, ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopF);
	SetDlgItemInt(IDC_EDIT_TRIGGER_STOPPOS_B, ((CNedLineCamera*)theApp.m_pCamera)->TriggerStopB);

	if (((CNedLineCamera*)theApp.m_pCamera)->TriggerCountF > BUFFER_COUNT)
		SetDlgItemInt(IDC_EDIT_TRIGGER_COUNT_F, BUFFER_COUNT);
	if (((CNedLineCamera*)theApp.m_pCamera)->TriggerCountB > BUFFER_COUNT)

#include "stdafx.h"
#include "VieworksComControl.h"
#include "define.h"

CVieworksComControl::CVieworksComControl(void)
{
	m_hComm=NULL;
}
...
bool CVieworksComControl::comSerialWrite(BYTE *pBuff, UINT32& Buffersize)
{
...
bool CVieworksComControl::comSerialRead(BYTE *pBuff, UINT32& Buffersize)
{
	unsigned long length;
...
int CVieworksComControl::SetCommand(char* Command,UINT32 TimeOut)
{
	...
	while (GetTickCount() - dwStart < TimeOut)
	{
		...
		while(1)
		{
			...
			if(bRet == false || numBytes == 0)	break;
	...
	if(rxData.GetLength()==0)return -22000; // no retuern string 
	else if(rxData.Find(_T("OK"),0)>=0 || rxData.Find(PCH2CSTR(Command),0)>=0 || (bRet == true && numBytes != 0))	return 0; // ok
	else if(rxData.Find(_T("Error"),0)>=0)return -1; // command Error 
	else return -2;

}


int CVieworksComControl::OpenComPort(UINT32 BaudRate, CString Portname)
{
	if(m_hComm!=NULL)return -21000;
...
void  CVieworksComControl::CloseComPort() 
{

...
BOOL CVSCameraTaskDlg::OnInitDialog()
{
	...
	// 버전 표기
	CString title;
	title.Format(_T("Matrox Camera %s"), _Version);
	SetWindowText(title);
	...
	if (((CMatroxCamera *)theApp.m_pCamera)->GetUseColorBayer())
	{
		// YSKIM - WB값 가져오기
		TCHAR strVal[11];
		...
		((CMatroxCamera *)theApp.m_pCamera)->SetWBCoefficient(WhiteBalance);
	}
	else
	{
		m_RedCoeff.EnableWindow(FALSE);
		m_GreenCoeff.EnableWindow(FALSE);
		m_BlueCoeff.EnableWindow(FALSE);
		GetDlgItem(IDC_BTN_SAVE_WB_VALUE)->EnableWindow(FALSE);
		GetDlgItem(IDC_BTN_CALC_RGB)->EnableWindow(FALSE);
	}
...
void CVSCameraTaskDlg::m_fnInitUIControls()
{
	...
	if (m_cmbCamModel.GetCurSel() == (int)eVA47M5_Series || m_cmbCamModel.GetCurSel() == (int)eVNP_MX_Series
		|| m_cmbCamModel.GetCurSel() == (int)eHuaRayPS || m_cmbCamModel.GetCurSel() == (int)eHuaRayNonPS)
	...
	if (m_cmbCamModel.GetCurSel() == (int)eHuaRayPS)
	{
		m_fnAddCmbString(m_cmbPSMode, strPSMode_HuaRay, (int)ePSMode_HuaRay_Count);
	}
	else
	{
...
void CVSCameraTaskDlg::m_fnSetAutoUIControls(CString strInitFilePath)
{
	...
	m_cmbCamModel.SetCurSel(GetPrivateProfileInt(_T("Settings"), _T("Camera Model"), 100, strInitFilePath) - 1);
...
// 	theApp.m_pCamera->TestFunc( m_nGV );
	...
	((CMatroxCamera*)theApp.m_pCamera)->OpenFeatureDialog();
}
...
{
	float WhiteBalance[3] = { 0, };
	((CMatroxCamera *)theApp.m_pCamera)->GetWBCoefficient(WhiteBalance);
